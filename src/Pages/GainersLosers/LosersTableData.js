...
const LosersTableData = (props) => {
  ...
  const { signMessage } = useSignMessage({
    onSuccess: async (data, variables) => {
      const result = await loserupdatePairAction(variables.message, data);
      await losergetLikeStatusByAccount(address).then(result => setLikeStatus(result));
    },
  });
...
export default LosersTableData;
